...
typedef struct {
    /* Owned storage. `size` bytes are meaningful; capacity may be larger. */
    uint8_t *data;
    size_t size;
    size_t capacity;
} ReplayBuffer;
...
/* Existing contents and size are preserved when reserve grows the allocation. */
...
/* Clear retains allocation for reuse; free releases it. */
