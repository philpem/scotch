...
GetWord (ARMul_State * state, ARMword address, int check)
{
  ...
  if (pageptr == NULL)
    {
      /* replay-tooling: pages are zero-filled (calloc, not malloc) so the
	 simulated address space reads as zero before being written, matching
	 the Unicorn harness this replaces.  */
      pageptr = (ARMword *) calloc (1, PAGESIZE);
...
PutWord (ARMul_State * state, ARMword address, ARMword data, int check)
{
  ...
  if (pageptr == NULL)
    {
      /* replay-tooling: zero-filled pages (see GetWord above).  */
      pageptr = (ARMword *) calloc (1, PAGESIZE);
      if (pageptr == NULL)
	{
	  perror ("ARMulator can't allocate VM page");
